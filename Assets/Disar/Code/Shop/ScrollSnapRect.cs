...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
   ...
   //------------------------------------------------------------------------
    void Start()
    {
        ...
        startingPage = PlayerPrefs.GetInt("Select_Record");
    ...
    void Update()
    {
        ...
        if (_currentPage==PlayerPrefs.GetInt("Select_Record"))
        {
            Sec.text = "Seçili";
            //Sec_btn.interactable = false;

        }
        else
        {
            Sec.text = "Seç";
            //Sec_btn.interactable = true;
        }
        ...
        for (int i = 0; i < _pageCount; i++)
        {
            ...
            if (_currentPage == i)
            {
                child.sizeDelta = new Vector2(120, 120);
            }
            else
            {
                child.sizeDelta = new Vector2(90, 90);
            }
        }
        

    }
    public void Select_Ball()
    {
        PlayerPrefs.SetInt("Select_Record", _currentPage);
    }
    //------------------------------------------------------------------------
    private void SetPagePositions()
    {
        ...
        if (_horizontal)
        {
            ...
            offsetX = width / 2;
            // total width
            containerWidth = width * _pageCount * Mathf.RoundToInt(Aralik);
        ...
        for (int i = 0; i < _pageCount; i++)
        {
            ...
            if (_horizontal)
            {
                childPosition = new Vector2(Aralik * i * width - containerWidth / 2 + offsetX, 0f);
            }
            else
            {
